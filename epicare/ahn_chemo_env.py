...
class AhnChemoEnv(gym.Env):
    ...
    def __init__(self, max_t: int = 600, delayed_steps=0,
        ...
        # 🚩 CHANGE: Action space is now discrete: {0, 1}
        self.action_space = spaces.Discrete(self.n_treatments)
    ...
    def reset(self, seed: int = None, **kwargs) -> Tuple[ObsType, dict[str, Any]]:
        ...
        self.reward_threshold = self.env_info['reward_range'][1]
    ...
    def step(self, action: float) -> tuple[ObsType, SupportsFloat, bool, bool, dict[str, Any]]:
        ...
        '''
        if action not in range(self.n_treatments):  # ✅ fixed check
            raise ValueError(f"action {action} must be in [0, {self.n_treatments-1}]")
        
        
        # 🚩 wrap binary action as float for ODEs
        if action == 0:
            u = 0.0
        else:
            u = 0.5 
        
        state_next = self.Simulator.update(action=u, state=self.cur_state)
        ...
        #return observation_next, reward, self.terminated,  info
        #return observation_next, reward, self.terminated, self.truncated, info
        return observation_next, reward, self.terminated or self.truncated, info
...
register(
    id="AhnChemo-v0",
    entry_point="epicare.ahn_chemo_env:AhnChemoEnv",
)

...
class BiasedPolicy_differentfeatures(BasePolicy):
    """
    Biased policy: assigns higher probability to specific treatments based on symptom values.
    Creates strong treatment selection bias.
    """

    def __init__(self, env, bias_strength=50.0):
        super().__init__(env)
        self.bias_strength = bias_strength  # Controls how "strong" the bias is

    def get_treatment_probs(self, observation):
        # For example, bias treatment i by symptom i 
        logits = np.zeros(self.env.n_treatments)
        n_symptoms = len(observation)
        for i in range(self.env.n_treatments):
            
            symptom_idx = i % n_symptoms
            logits[i] = self.bias_strength * observation[symptom_idx]
        
        exp_logits = np.exp(logits - np.max(logits))
        probs = exp_logits / exp_logits.sum()
        return probs

class BiasedPolicy_Ahn(BasePolicy):
    def get_treatment_probs(self, observation):
       
        T, I, B = observation
        logits = np.array([0.0, 0.0])
        # bias towards treatment if T high & I good & B low
        logits[1] = 5*(T - 0.5) - 3*(B - 0.5) + 2*(I - 0.5)
        logits[0] = 0
        exp_logits = np.exp(logits - np.max(logits))
        probs = exp_logits / exp_logits.sum()

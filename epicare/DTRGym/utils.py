...
class DiscreteActionWrapper(Wrapper):
    def __init__(self, env, n_act: int):
        ...
        self.env.env_info["action_type"] = "discrete"
        
    def seed(self, seed=None):
        # forward seed to the wrapped env
    ...
    def get_normalized_score(self, eval_scores):
        return self.env.get_normalized_score(eval_scores)

class PatchedDiscreteActionWrapper(DiscreteActionWrapper):
    def step(self, action: int):
        # explicitly set the bins
        act_bin = np.array([0.0, 2.0])
        return self.env.step(np.array([act_bin[action]]))

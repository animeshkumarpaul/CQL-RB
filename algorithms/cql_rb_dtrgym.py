"""
CQL-RB: Conservative Q-Learning with Representation Balancing
=============================================================

This implementation builds on the open-source baseline code from:

    EpiCare GitHub Repository
    https://github.com/Grosenick-Lab-Cornell/EpiCare

Original Components:
--------------------
- The underlying CQL (Conservative Q-Learning) implementation is based on:
...
NEW in This Code:
---------------------------------
This implementation introduces **Representation Balancing (RB)** into the CQL framework:
    - Balances treated vs. control (or across treatment groups) using **IPM/MMD-based regularization**.
    - Inspired by domain adaptation methods and Shalit et al., "Estimating Individual Treatment Effect: Generalization Bounds and Algorithms." ICML 2016.
    - Controlled by hyperparameter `beta_ipm`.

Modular integration with CFRNet-style architectures to support:
    - Encoder-based feature representations.
    - Joint optimization of Bellman error and distribution alignment.


Usage Notes:
------------
This file is intended for use in offline RL experiments on biased healthcare datasets using the dtrgym framework.
...
from epicare.ahn_chemo_env import AhnChemoEnv
import wandb
...
class TrainConfig:
    ...
    beta_ipm: float = 100.0 # cfrnet IPM loss
    batch_size: int = 256  # Batch size for all networks
    buffer_size: int = 1000000  # Replay buffer size
    checkpoints_path: Optional[str] = "./algorithms/cancer/CQL_rb/checkpoints_" # Save path
    dataset_path: Optional[str] = None  # Path to the dataset
    device: str = "cuda:6"
    env: str = "AhnChemo-v0"  # OpenAI gym environment name
    env_seed: int = 1  # Environment seed
    episodes_avail: int = 15000  # Number of episodes
    eval_freq: int = 25000  # How often (time steps) we evaluate
    frame_stack: int = 20  # Number of frames to stack
    gamma: float = 0.95  # Discount factor
    load_model: str = ""  # Model load file name, "" doesn't load
    n_episodes: int = 5000  # How many episodes run during evaluation
    ...
    name: str = "CQL_RB"
    project: str = "CQL-dtrgym"
    group: Optional[str] = ""  # DEPCRECATED
    behavior_policy: str = "biased_ahnChemo"  # Behavior policy for data collection
    ...
    def update_params(self, params: Dict[str, Any]) -> "TrainConfig":
        ...
        for key, value in params.items():
            if hasattr(self, key):
                ...
                try:
                    setattr(self, key, attr_type(value['values'][0]))
                except (ValueError, TypeError):
                    print(f"[Warning] Could not convert {key}='{value}' to {attr_type}")
                #setattr(self, key, value)
            else:
                print(f"[Warning] Unknown config key: {key}")
        '''
        
        print("self.behavior_policy", self.behavior_policy)
        print("self.qf_lr", self.qf_lr)
        self.dataset_path = (
            f"./data/{self.behavior_policy}/train_seed_{self.env_seed}.hdf5"
        )
        self.checkpoints_path  = self.checkpoints_path + self.behavior_policy
...
def init_module_weights(module: torch.nn.Sequential, orthogonal_init: bool = False, rep_init:bool = True):
    # Specific orthgonal initialization for inner layers
    # If orthogonal init is off, we do not change default initialization
    if orthogonal_init and rep_init:
...
class FullyConnectedQFunction(nn.Module):
    def __init__(
        ...
        for _ in range(n_hidden_layers - 1):
            rep_layers.append(nn.Linear(256, 256))
            rep_layers.append(nn.ReLU())
        self.rep = nn.Sequential(*rep_layers)
         
        q_layers = [nn.Linear(256, action_dim)]
        #layers.append(nn.Linear(256, action_dim))

        self.q_head = nn.Sequential(*q_layers)
        init_module_weights(self.rep, orthogonal_init)
        init_module_weights(self.q_head, orthogonal_init, rep_init=False)

    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        rep_out = self.rep(observations)
        return self.q_head(rep_out)
...
class DiscreteCQL:
    def __init__(
        ...
        self.use_ipm = True
        self.alpha = alpha
        self.beta_ipm = beta_ipm
        self.ipm_function = "mmd"
        self.device = device
    ...
    def train(self, batch: TensorBatch) -> Dict[str, float]:
        ...
        """
        Computes the total loss for CQL-RB.

        Components:
        1. Bellman error (MSE between Q_pred and Q_target)
        2. Conservative penalty to avoid overestimation (CQL term)
        3. Representation balancing penalty (e.g., MMD/IPM)
        """
        ...
        representation_output = self.q1.rep(observations) 
        treatment_type = actions.argmax(dim=1)  
        rep_treated1, rep_control1 = get_data_with_treatment_type(representation_output, treatment_type)

        representation_output = self.q2.rep(observations) 
        treatment_type = actions.argmax(dim=1)  
        rep_treated2, rep_control2 = get_data_with_treatment_type(representation_output, treatment_type)

        
        
        if self.use_ipm:
            if self.ipm_function == "mmd":
                ipm_loss1 = maximum_mean_discrepancy_loss(rep_treated1, rep_control1)
                ipm_loss2 = maximum_mean_discrepancy_loss(rep_treated2, rep_control2)
                ipm_loss = self.beta_ipm *  (ipm_loss1 + ipm_loss2) 
                
                log_dict["ipm_loss1"] = ipm_loss1.item()
                log_dict["ipm_loss2"] = ipm_loss2.item()
                
            elif self.ipm_function == "wasserstein":
                ipm_loss = self.beta_ipm * wasserstein_distance(rep_treated, rep_control)
            else:
        ...
        loss = dqn_loss + self.alpha * conservative_loss + ipm_loss
...
def train(config: TrainConfig):
    ...
    print("---------------------------------------")
    print("Alpha IPM value: ", config.beta_ipm)
    # Initialize trainer
    trainer = DiscreteCQL(
        q1,
        q1_optimizer,
...
if __name__ == "__main__":
    ...
    if args.subcommand == "eval":
        ...
        #run = wandb.init(config=sweep_config['parameters'])
